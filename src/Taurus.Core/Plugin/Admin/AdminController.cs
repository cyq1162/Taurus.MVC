...
namespace Taurus.Plugin.Admin
{
    ...
    internal partial class AdminController : Controller
    {
        ...
        public override bool BeforeInvoke()
        {
            ...
            switch (nameLower)
            {
                case "login":
                    return true;
                default:
                    if (!AdminAPI.Auth.IsOnline(this.Context, !isNoViewEvent))
                    {
        ...
        private string GetMsTypeText()
        {
            ...
            {
                return "Registry Center of Master" + (MsConfig.Server.IsEnable ? " ( Running )" : " ( Stopped )");
            }
            else if (MsConfig.IsRegistryCenter)
            {
                return "Registry Center of Slave" + (MsConfig.Server.IsEnable ? " ( Running )" : " ( Stopped )") + (Server.IsLiveOfMasterRC ? "" : " - ( Master connection refused )");
            }
            else if (MsConfig.IsGateway)
            {
                return "Gateway" + (MsConfig.Server.IsEnable ? " ( Running )" : " ( Stopped )") + (Server.IsLiveOfMasterRC ? "" : " - ( Registry center connection refused )");
            }
            else if (MsConfig.IsClient)
            {
                return "Client of MicroService" + (MsConfig.Client.IsEnable ? " ( Running )" : " ( Stopped )") + (Client.IsLiveOfMasterRC ? "" : " - ( Registry center connection refused )");
            }
            else
            {
                return "None";
        ...
        protected bool IsAdmin
        {
            get
            {
                return AdminAPI.Auth.GetIsAdmin(this.Context);
            }
        }
        protected string LoginName
        {
            get
            {
                return AdminAPI.Auth.GetLoginName(this.Context);
            }
        }
    }
}
